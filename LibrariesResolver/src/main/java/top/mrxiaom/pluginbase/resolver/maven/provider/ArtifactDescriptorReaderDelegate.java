...
import top.mrxiaom.pluginbase.resolver.aether.artifact.*;
import top.mrxiaom.pluginbase.resolver.aether.graph.Dependency;
import top.mrxiaom.pluginbase.resolver.aether.graph.Exclusion;
...
public class ArtifactDescriptorReaderDelegate {
    public void populateResult(RepositorySystemSession session, ArtifactDescriptorResult result, Model model) {
        ...
        for (top.mrxiaom.pluginbase.resolver.maven.model.Dependency dependency : model.getDependencies()) {
            result.addDependency(convert(dependency, stereotypes));
        }

        DependencyManagement mgmt = model.getDependencyManagement();
        if (mgmt != null) {
            for (top.mrxiaom.pluginbase.resolver.maven.model.Dependency dependency : mgmt.getDependencies()) {
    ...
    private Dependency convert(top.mrxiaom.pluginbase.resolver.maven.model.Dependency dependency, ArtifactTypeRegistry stereotypes) {
        ...
        for (top.mrxiaom.pluginbase.resolver.maven.model.Exclusion exclusion : dependency.getExclusions()) {
    ...
    private Exclusion convert(top.mrxiaom.pluginbase.resolver.maven.model.Exclusion exclusion) {
